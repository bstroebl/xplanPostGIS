...
CREATE OR REPLACE FUNCTION "QGIS".imp_create_schema(
    nspname character varying, ueberschreiben integer)
    RETURNS integer AS
$BODY$
BEGIN
    IF ueberschreiben = 1 THEN
...
LANGUAGE 'plpgsql' VOLATILE
COST 100;
GRANT EXECUTE ON FUNCTION "QGIS".imp_create_schema(character varying,integer) TO xp_user;
COMMENT ON FUNCTION "QGIS".imp_create_schema(character varying,integer)
